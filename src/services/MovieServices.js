...
export const GetMovieDetails = async (id) => {
    try {
        ...
        const reviews = await Client.get(`/movie/${id}/reviews${API_KEY}`)
        const returnData = {movie: response.data, stream: streaming.data.results.US, recommend: recommendations.data.results.filter(r => r.poster_path!==null), reviews: reviews.data.results}
        return returnData
    } catch(err) {console.log(err)}
}
